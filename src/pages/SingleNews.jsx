...
function SingleNews() {
  // substituir por dados do backend
  const dataNews = [
    {
      id: 1,
      date: "2023-09-18",
    ...
    },
    {
      ...
      text: "    Lorem, ipsum dolor sit amet consectetur adipisicing elit. Labore perferendis totam quaerat doloribus repellat quam nostrum animi iste porro quis nihil unde voluptatum esse, numquam nisi tempora obcaecati inventore quidem. Lorem ipsum dolor sit amet consectetur adipisicing elit. Suscipit vero aliquid amet, deleniti pariatur provident corrupti molestiae id tempora harum minus ea est hic, quod minima! Quas nisi facere velit. Pernambuco abriga uma rica biodiversidade, mas muitas espécies estão ameaçadas de extinção devido à perda de habitat. O estado está intensificando os esforços de conservação, estabelecendo áreas protegidas e promovendo a educaçentore quidem. Lorem ipsum dolor sit amet consectetur adipisicing elit. Suscipit vero aliquid amet, deleniti pariatur provident corrupti molestiae id tempora harum minus ea est hic, quod minima! Quas nisi facere velit. Pernambuco abriga uma intensificando os esforços de conservação, estabelecendo áreas protegidas e promovendo a educação ambiental ão ambiental para preservar sua riqueza natural."
    }
  ];

  const { id } = useParams();
  const news = dataNews.find((news) => news.id === Number(id));
  ...
  return (
    <>
      ...
      <div className='news-pg fade-in'>
        ...
        <p className='news-text'><div><img id='news-text-img' src={news.img} alt={news.alt} />
          <span>{news.alt}</span></div>{news.text}</p>
      </div>
      <Footer />
    </>
  );
